...
/** Deterministic fixed-camera projection shared by local thumbnails and server preview generation. */
public final class SchematicThumbnailProjection {
    private SchematicThumbnailProjection() {
    }

    public static void render(SchematicThumbnail thumbnail, float width, float height, float padding,
    ...
    private static void drawColumn(QuadSink sink, float cx, float cy, float halfW, float halfH,
                                   float sideDepth, MaterialColors colors, float heightRatio) {
        float topY = cy - halfH;
    ...
    public record MaterialColors(int top, int left, int right) {
        public static MaterialColors uniform(int color) {
    ...
    public interface MaterialResolver {
        MaterialColors resolve(String state, int fallbackColor);
    }

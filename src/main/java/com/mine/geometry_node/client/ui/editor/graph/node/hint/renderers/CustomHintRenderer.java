...
public class CustomHintRenderer implements UIHintRenderer {
    ...
    public View createView(Context context, NodeData nodeData, PortRow row, EditorContext editorContext) {
        ...
        if (ColorRamp.GRADIENT_INPUT.equals(row.customWidgetId())) {
            return new ColorRampView(context, nodeData, editorContext);
        }
        if (DataLibraryReference.INFO_WIDGET_ID.equals(row.customWidgetId())) {
            return new DataLibraryReferenceInfoView(context, nodeData, editorContext);
        }
        return null;
    }
    ...
    public void updateLayout(View view, PortRow row, float currentY, int nodeWidth) {
        ...
        float height = DataLibraryReference.INFO_WIDGET_ID.equals(row.customWidgetId())
                ? DataLibraryReferenceInfoView.HEIGHT_DP : ColorRampView.HEIGHT_DP;
        int heightPx = UIUtils.dp2pxInt(height);
    ...
    public float getRequiredExtraRows(PortRow row) {
        ...
        if (row != null && DataLibraryReference.INFO_WIDGET_ID.equals(row.customWidgetId())) {
            return Math.max(1.0f, DataLibraryReferenceInfoView.HEIGHT_DP / (float) UIConstants.Node.ROW_HEIGHT);
        }

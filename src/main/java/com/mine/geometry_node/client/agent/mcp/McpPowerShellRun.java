...
/** Owns one graph-scoped MCP registration for the lifetime of a PowerShell process. */
public final class McpPowerShellRun implements AutoCloseable {
    ...
    private McpPowerShellRun(McpHttpServer mcp, ProcessLaunchSpec launchSpec) {
        this.mcp = mcp;
    ...
    public static McpPowerShellRun start(CommandRegistry registry, McpCommandGateway gateway,
        ...
        McpRequestDispatcher dispatcher = new McpRequestDispatcher(
                new McpToolCatalog(registry), gateway, eventListener);
        ...
        try {
            ...
            return new McpPowerShellRun(mcp, launch);
    ...
    @Override
    public void close() {
        if (closed.compareAndSet(false, true)) mcp.close();
    }
}

...
- **AI-assisted editing and MCP** — Expose authenticated, loopback-only MCP tools that let compatible AI clients inspect editor and graph state, query node contracts, and submit reversible graph patches. Authorized changes are transaction-planned and committed atomically as one undoable edit.
